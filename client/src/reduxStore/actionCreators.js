import { registerIgnorableActions } from '../reduxMiddleware/ignoreActionsMiddleware';
...
export function createRequestSuccessAndFailureActions(baseType) {
	...
	const actions = {
		request: (payload, meta = {}) =>
			createAction(types.request, payload, meta),
		success: (request, response, meta = {}) =>
			createAction(types.success, { request, response }, meta),
		failure: (request, error, meta = {}) =>
			createAction(types.failure, { request, error }, meta),
	};
 ...
 */
export function createSagaActions(namespace, type) {
	const actions = createRequestSuccessAndFailureActions(
		`${namespace}_SAGA_${type}`
	);

	registerIgnorableActions([
 ...
 */
export function createStoreAction(namespace, type) {
	const storeType = `${namespace}_STORE_${type}`;

	const action = (payload, meta = {}) =>
		createAction(storeType, payload, meta);
